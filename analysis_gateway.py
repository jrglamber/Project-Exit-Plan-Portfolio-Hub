...
ANALYSIS_GATEWAY_VERSION = "1.25.0"
...
def _emit_transition_confirmation_study() -> None:
    """Bridge point-in-time transition confirmation evidence."""
    try:
        payload = _fetch_producer_endpoint("indices", "/analysis/transition-confirmation-study")
        print("PEP_TRANSITION_CONFIRMATION_STUDY " + json.dumps({
            "gateway_version": ANALYSIS_GATEWAY_VERSION,
            "read_only": True, "execution_authority": False,
            "source": "indices", "payload": payload,
        }, separators=(",", ":"), default=str), flush=True)
    except Exception as exc:
        print("PEP_TRANSITION_CONFIRMATION_STUDY_ERROR indices " + type(exc).__name__ + ": " + str(exc), flush=True)



def _emit_protection_persistence_study() -> None:
    """Bridge protection persistence economics."""
    try:
        payload = _fetch_producer_endpoint("indices", "/analysis/protection-persistence-study")
        print("PEP_PROTECTION_PERSISTENCE_STUDY " + json.dumps({
            "gateway_version": ANALYSIS_GATEWAY_VERSION, "read_only": True,
            "execution_authority": False, "source": "indices", "payload": payload,
        }, separators=(",", ":"), default=str), flush=True)
    except Exception as exc:
        print("PEP_PROTECTION_PERSISTENCE_STUDY_ERROR indices " + type(exc).__name__ + ": " + str(exc), flush=True)



def _emit_rolling_hwm_causal_study() -> None:
    """Bridge causal rolling-HWM protection study."""
    try:
        payload = _fetch_producer_endpoint("indices", "/analysis/rolling-hwm-causal-study")
        print("PEP_ROLLING_HWM_CAUSAL_STUDY " + json.dumps({
            "gateway_version": ANALYSIS_GATEWAY_VERSION, "read_only": True,
            "execution_authority": False, "source": "indices", "payload": payload,
        }, separators=(",", ":"), default=str), flush=True)
    except Exception as exc:
        print("PEP_ROLLING_HWM_CAUSAL_STUDY_ERROR indices " + type(exc).__name__ + ": " + str(exc), flush=True)
...
def _emit_mature_hwm_causal_study() -> None:
    try:
        ...
        print("PEP_MATURE_HWM_CAUSAL_STUDY " + json.dumps({
            "gateway_version": ANALYSIS_GATEWAY_VERSION, "read_only": True,
            "execution_authority": False, "source": "indices", "payload": payload,
        }, separators=(",", ":"), default=str), flush=True)
    except Exception as exc:
        print("PEP_MATURE_HWM_CAUSAL_STUDY_ERROR indices " + type(exc).__name__ + ": " + str(exc), flush=True)
    ...
    while True:
        ...
        if cycle % discovery_every == 0 or producer_changed:
            try:
                ...
                _emit_transition_confirmation_study()
